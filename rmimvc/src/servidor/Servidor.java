...
public class Servidor extends ServidorRMI {
	 ...
	 * @throws RMIMVCException error lanzado por problemas registrando el modelo en el servidor RMI.
	 * @see RemoteException
	 */
	public <T extends Remote> T iniciar(T modelo) throws RemoteException, RMIMVCException {
		this.iniciarServidorRMI();
		try {
			return this.exportarObjeto("MVCRMI/Modelo", modelo);
		} catch (AlreadyBoundException e) {
			throw new RMIMVCException("Error al registrar el modelo en el servidor RMI local, ya se encuentra registrado.");
		}
	}
}
